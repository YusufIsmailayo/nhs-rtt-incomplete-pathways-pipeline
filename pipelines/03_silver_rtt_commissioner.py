"""
Silver Layer — RTT Commissioner (Incomplete Pathways)
======================================================
Loops through all Bronze commissioner Parquet files.
...
DIMENSION_COLS = ["treatment_function_code", "treatment_function"]
...
def process_month(bronze_path: Path):
    month = bronze_path.stem.replace("bronze_rtt_commissioner_", "")
    print(f"\n  [{month}]")
    ...
    if missing:
        ...
        print(f"    Dimensions: OK | Shape: {df.shape}")

    out = SILVER_DIR / f"silver_rtt_commissioner_{month}.parquet"
    df.to_parquet(out, index=False)
    print(f"    Saved → {out.relative_to(PROJECT_ROOT)}")


def main():
    print("\n── Silver: RTT Commissioner ─────────────────────────────────")

    bronze_files = sorted(BRONZE_DIR.glob("bronze_rtt_commissioner_*.parquet"))
    if not bronze_files:
        raise FileNotFoundError("No Bronze commissioner files found. Run 01_bronze_rtt_loader.py first.")

    print(f"  Found {len(bronze_files)} month(s)")
    for f in bronze_files:
        process_month(f)

    print("\n── Silver commissioner complete ─────────────────────────────\n")
